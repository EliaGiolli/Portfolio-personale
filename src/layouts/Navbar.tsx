...
function Navbar() {
    ...
    return (
        <header className={`flex justify-around items-center w-full h-28 py-3 px-4 sticky top-0 z-50 
            ${themeMode === 'dark' ? 'bg-gray-800 text-gray-300' : 'bg-blue-50 text-gray-700'}`}>
            <nav className='flex justify-center max-w-[1200px] mx-auto px-4 gap-10'>
                ...
                <ul className='hidden sm:flex justify-end items-center gap-4'>
                    <li className={`${themeMode === 'dark' ? 'text-gray-300 hover:bg-cyan-800 hover:text-white hover:rounded-lg' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-600 hover:rounded-lg'} text-lg md:text-xl px-3 py-2 transition-colors duration-200 ease-in-out`}>
                ...
                <div className='flex items-center gap-4'>
                    ...
                    <button 
                        ...
                        >
                        <FaBars className={`text-3xl ${themeMode === 'dark' ? 'text-white': 'text-gray-900'}`}/>
                    </button>
                </div>
                ...
                {isDropDown && (
                    <div 
                    ref={mobileMenuRef}
                    className={`absolute top-full right-0 w-full sm:hidden p-8 z-50 border-b-2 border-t-2 ${themeMode === 'dark' ? 'bg-gray-800 border-cyan-800' : 'bg-white border-gray-200'}`}
                    role="menu"
                    aria-label="Mobile navigation menu"
                    >
                        <ul className='flex flex-col gap-4'>
                            <li className={`${themeMode === 'dark' ? 'text-gray-300 hover:text-white' : 'text-gray-900 hover:text-blue-600'} text-lg md:text-xl transition-colors duration-200 ease-in-out`}>
